...
- **Backend binding — spike in phase 1, decided.** Operator structs with `next()` procedures, no closures, no dynamic dispatch on the hot path (STORE-A-0002 precedent: procedure-pointer adapters in test code only). The two candidate mechanisms — generic $-param executor monomorphized per backend vs. backend-independent core with thin `sparql/memstore` / `sparql/kvstore` instantiation packages — are prototyped against the real BGP join in the first evaluation task; the winner is recorded here before further operator tasks decompose.
