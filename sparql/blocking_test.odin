...
// where it was because there was an instantiation to test. Not one
// assertion changed: none of them was ever about a backend. The store
// harness and the query driver are in `testkit_test.odin` now.)*
...
//
// The store is closed before the rows are compared — see
// test_solve_source — so every term below has already outlived the
// dictionary arena it was decoded from.
@(private = "file")
solutions :: proc(
	t: ^testing.T,
	source: string,
	...
	return test_solve_source(t, source, query, render_solution, loc)
}
